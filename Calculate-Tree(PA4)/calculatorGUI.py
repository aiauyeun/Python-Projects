...
# date: 11/18/22
# file: calculatorGUI.py creates a simple calculator interface using tkinter that calls the calculate function from calculate.py
# input: button presses to make an infix equation
# output: solved equation displayed on calculator
from tkinter import *
...
def calculator(gui):   
    ...
    b_lbrak = addButton(gui,entrybox,'(')
    b_rbrak = addButton(gui,entrybox,')')
    ...
    buttons =[ b7,    b8, b9,    b_clr, 
               b4,    b5, b6,    b_sub, 
               b1,    b2, b3,    b_add, 
               b_mult,b0, b_div, b_exp,
               b_lbrak, b_rbrak, b_decimal, b_eq ]
    k = 4
    l = 5           
    for i in range(l):
        for j in range(k):
...
def clickButton(entrybox, value):
    ...
    if value == 'c':
        ...
        try:
            ...
            entrybox.insert(0,str(calculate(eq)))
        except:
            entrybox.insert(0,'ERROR! PRESS C TO CLEAR MESSAGE')
    else:
